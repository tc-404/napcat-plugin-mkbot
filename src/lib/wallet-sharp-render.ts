...
function buildWalletSvg(width: number, height: number, opts: WalletSharpRenderOptions): string {
  ...
  parts.push(`<defs>
    ...
    <filter id="valGlowPurple" x="-35%" y="-35%" width="170%" height="170%">
      <feDropShadow dx="0" dy="0" stdDeviation="3" flood-color="#ede7f6" flood-opacity="0.6"/>
      <feDropShadow dx="0" dy="0" stdDeviation="6" flood-color="#b388ff" flood-opacity="0.9"/>
      <feDropShadow dx="0" dy="2" stdDeviation="10" flood-color="#7c4dff" flood-opacity="0.4"/>
    </filter>
  </defs>`);
  ...
  const tiles = [
    ...
    { label: '禁言卡', value: muteCardCount, grad: 'purpleGrad', glow: 'valGlowPurple', unit: '张' },
  ];
  ...
  tiles.forEach((tile, i) => {
    ...
    parts.push(
      `<rect x="${x}" y="${y}" width="${tileW}" height="${tileH}" rx="18" ry="18" fill="rgba(90,20,60,0.10)" stroke="rgba(255,182,193,0.32)" stroke-width="2"/>`,
    );
    parts.push(
      `<circle cx="${x + 22}" cy="${y + 24}" r="5" fill="${accent}"/>`,
    );
    parts.push(
      `<text x="${x + 36}" y="${y + 28}" font-family="${font}" font-size="16" font-weight="900" fill="#ffffff">${escapeXml(tile.label)}</text>`,
    );
    parts.push(
      `<text x="${x + 22}" y="${y + 78}" font-family="${font}" font-size="${valSize}" font-weight="900" fill="url(#${tile.grad})" filter="url(#${tile.glow})">${escapeXml(displayValue)}</text>`,
    );
  });
