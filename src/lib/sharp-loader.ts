// ---------------------------------------------------------------------------
// Sharp 模块加载：优先从 data/runtime-deps（避免咔咔珂 plugins 热重载打断 npm install）
// ---------------------------------------------------------------------------
...
export interface SharpRuntimePaths {
  /** 插件数据目录（咔咔珂: data/plugins/kakake-plugin-mkbot） */
  dataDir?: string;
  /** 插件代码目录（咔咔珂: plugins/kakake-plugin-mkbot） */
  pluginDir?: string;
}

/** runtime-deps 子目录名：npm install 目标，不在 plugins 目录内 */
...
/** 解析 Sharp 依赖安装/检测目录（优先 data/runtime-deps） */
