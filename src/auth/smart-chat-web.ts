...
export function registerSmartChatWebGetRoutes(
    ...
    base.get(wrapPath('/smart-chat/knowledge'), (req, res) => {
        try {
            ...
            res.json({ code: 0, data: { scope, entries: listKnowledge(deps, scope), scopes: listKnowledgeScopes(deps) } });
        } catch (e) {
            logger?.error?.('获取知识库失败:', e);
            res.status(500).json({ code: -1, message: '获取知识库失败' });
        }
    });
...
export function registerSmartChatWebPostRoutes(
    ...
    base.post(wrapPath('/smart-chat/knowledge/upsert'), async (req, res) => {
        try {
            ...
            logger?.error?.('保存知识库条目失败:', e);
            res.status(500).json({ code: -1, message: '保存知识库条目失败' });
        }
    });
    ...
    base.post(wrapPath('/smart-chat/knowledge/toggle'), async (req, res) => {
        try {
            ...
            logger?.error?.('切换知识库开关失败:', e);
            res.status(500).json({ code: -1, message: '切换知识库开关失败' });
        }
    });
    ...
    base.post(wrapPath('/smart-chat/knowledge/delete'), async (req, res) => {
        try {
            ...
            logger?.error?.('删除知识库条目失败:', e);
            res.status(500).json({ code: -1, message: '删除知识库条目失败' });
        }
    });
